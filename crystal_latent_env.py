...
class CrystalLatentEnv(gym.Env):
    ...
    def _decode_latent(self, z: np.ndarray, step_index: int) -> Tuple[np.ndarray, float, Dict]:
        ...
        cif_path = None
        pred_formula = None  # ensure defined even if conversion fails
        if self.save_cif:
            ...
            (
                ...
                print_error=True,   # turn on while debugging
            )

            # Find the CIF that was actually written (if any)
            cifs = sorted(glob.glob(os.path.join(cif_folder, "*.cif")))
            if cifs:
                cif_path = cifs[0]

                # Create POSCAR only if CIF exists
                structure = Structure.from_file(cif_path)
                structure.to(filename=os.path.join(cif_folder, "POSCAR"))
            else:
                # No CIF produced (likely non-convertible decode)
                cif_path = None
